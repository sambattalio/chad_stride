...
bool RAINBOW = false, NORAINBOW = false, STAY = false, LOOP = false, NOFUCKS = false, COLOR = false, MES = false, REVERSE = false, FLIP = false, LABEL = false;
...
const char* chad_frames_LABEL[CHAD_FRAMES][CHAD_ROWS] = {
    {
        "                          %",
        "                     .(....",
        "                  %......(",
...
int main(int argc, char *argv[]) {
    ...
    while(x_pos != endpoint || LOOP || FLIP) {
		...
		if(LABEL){
			...
			//chad_frames_LABEL[i%CHAD_FRAMES][j];

			for (int j = 0; j < CHAD_ROWS; j++) {
              // find position of furthest character to the right
              int right_pos = x_pos + strlen(chad_frames_LABEL[i % CHAD_FRAMES][j]);
              // draw row, checking it doesn't go off screen if not looping
              mvaddnstr(j * SIZE + row/2 - (CHAD_ROWS*SIZE/2), x_pos,
                  chad_frames_LABEL[i % CHAD_FRAMES][j],
                  (right_pos > col && !LOOP) ? col - x_pos : right_pos);
